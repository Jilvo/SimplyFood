...
class ModelTest(TestCase):
    def setUp(self):
        # Create some users
        self.user_1 = User.objects.create_user('Chevy Chase', 'chevyspassword','chevy@chase.com')
        self.user_2 = User.objects.create_user('Jim Carrey', 'jimspassword','jim@carrey.com')
        ...
        self.macaroni = Recipe.objects.create(name="Macaroni Cheese",description_list="c'est des pâtes")
    ...
    def test_add_list(self):
        """test_add_list"""
        test_add_list = User_Recipe_list.objects.create(user_name=self.user_1,list_recipe="['Steak Teriyaki ', 'Poulet persan ','Macaroni Cheese']")
        test_add_list.save()
        test_find_list  = User_Recipe_list.objects.get(list_recipe="['Steak Teriyaki ', 'Poulet persan ','Macaroni Cheese']")
        t_t = test_find_list
        print(t_t)
...
class RegisterTestCase(TestCase):
    ...
    def test_cant_register_user_with_unmatching_passwords(self):
        response=self.client.post(self.login_url,self.user_wrong_pwd,format='text/html')
        self.assertEqual(response.status_code,302)

    def test_cant_register_user_with_invalid_email(self):
        response=self.client.post(self.login_url,self.user_wrong_email,format='text/html')
        self.assertEqual(response.status_code,302)
